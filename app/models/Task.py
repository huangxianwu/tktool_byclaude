...
class Task(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'created_at': TimezoneHelper.utc_to_local(self.created_at).isoformat() if self.created_at else None,
            'timeout_at': TimezoneHelper.utc_to_local(self.timeout_at).isoformat() if self.timeout_at else None,
            'started_at': TimezoneHelper.utc_to_local(self.started_at).isoformat() if self.started_at else None,
            'completed_at': TimezoneHelper.utc_to_local(self.completed_at).isoformat() if self.completed_at else None,
